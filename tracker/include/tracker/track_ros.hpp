...
public:
    Track_Ros(int fps, ros::NodeHandle nh)
    {
        ...
        sub = nh.subscribe<rockauto_msgs::DetectedObjectArray>("/vision_nodelet_manager/sync/fused_msg", 10, &Track_Ros::callback, this);

        // ros::AsyncSpinner spinner(1);
        // spinner.start();
        camera_pub = nh.advertise<sensor_msgs::Image>("trackImage", 10);
        chatter_pub = nh.advertise<rockauto_msgs::DetectedObjectArray>("chatter", 10);
        ros::spin();
    }
...
private:
    ...
    void callback(const rockauto_msgs::DetectedObjectArray::ConstPtr &msg);
